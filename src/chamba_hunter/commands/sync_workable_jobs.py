...
from chamba_hunter.services.workable_job_ingestion_service import (
    WorkableJobIngestionService,
)
...
def main() -> None:
    ...
    if args.account_subdomain is not None:
        requested_account = (
            args.account_subdomain.strip()
        )
        ...
        if not company_ats_records:
            parser.error(
                "No active primary Workable "
                "ATS found for account "
                f"subdomain '{args.account_subdomain}'."
            )
    ...
    print("Syncing Workable jobs...")
    print(
        f"Accounts: {len(company_ats_records)}"
    )
    print()

    service = WorkableJobIngestionService(

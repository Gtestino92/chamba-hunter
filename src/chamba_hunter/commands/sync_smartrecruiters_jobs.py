...
from chamba_hunter.services.smartrecruiters_job_ingestion_service import (
    SmartRecruitersJobIngestionService,
)
...
def main() -> None:
    ...
    if args.company_identifier is not None:
        requested_identifier = (
            args.company_identifier.strip()
        )
        ...
        if not company_ats_records:
            parser.error(
                "No active primary "
                "SmartRecruiters ATS found "
                "for company identifier "
                f"'{args.company_identifier}'."
            )
